...
<div class="flex flex-col gap-stack-lg max-w-2xl mx-auto">
    <form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data" class="bg-surface-white rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.02)] border border-border-subtle p-6 flex flex-col gap-stack-lg">
        @csrf
        ...
        <div class="flex flex-col gap-stack-sm">
            ...
            <input
                ...
                placeholder="Contoh: Indomilk Coklat 250ml"
            />
            @error('name')
                <span class="text-label-md text-error">{{ $message }}</span>
            @enderror
        ...
        {{-- Barcode --}}
        <x-barcode-input name="barcode" :value="old('barcode')" label="Barcode *" />
